...
def shipped_timeline(metric: pd.DataFrame, frames: np.ndarray):
    ...
    Not a reimplementation of it. The flight gate was measured here and then
    added to `stats`, and two ways of smoothing the ball's speed are enough
    to make those different rules -- so the scorer calls the shipped
    function, and if someone changes it this number moves.
    """
    held = stats.possession_timeline(metric)
    if held.empty:
        return np.full(frames.shape, None, dtype=object)
    by_frame = dict(zip(held.frame.astype(int), held.team))
    return np.array([by_frame.get(int(f)) for f in frames], dtype=object)
...
def main():
    ...
    for out_dir, (source, offset) in CLIP_SOURCES.items():
        ...
        for label, predicted in (
                ...
                ("shipped", shipped_timeline(metric, frames))):
            for gap in GAPS_S:
